...
export default function Home() {
  ...
  return (
    <div className="flex h-screen bg-[#F5F5F0] dark:bg-black text-gray-800 dark:text-gray-200 font-sans transition-colors overflow-hidden">
      
      {/* Background Orbs */}
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-[#7E8D6B]/10 dark:bg-[#5A664B]/10 blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full bg-stone-500/10 dark:bg-stone-600/10 blur-[120px] pointer-events-none" />
      ...
      <aside className="w-72 flex-shrink-0 border-r border-gray-200/50 dark:border-white/5 bg-white/50 dark:bg-black/20 backdrop-blur-xl flex-col hidden md:flex z-10">
        <div className="p-5 border-b border-gray-200/50 dark:border-white/5">
          <button 
            ...
            className="cursor-pointer flex items-center justify-center gap-2 w-full px-4 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-[#5A664B] to-[#434B38] rounded-xl hover:from-[#434B38] hover:to-[#33392A] shadow-sm transition-all transform active:scale-95"
          >
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {isConversationsLoading ? (
            <div className="flex justify-center p-4">
              <div className="w-5 h-5 border-2 border-[#5A664B] border-t-transparent rounded-full animate-spin"></div>
            </div>
              ...
              {conversations?.map((conv) => (
                <button
                  ...
                  className={`cursor-pointer w-full text-left px-3 py-3 rounded-xl transition-all flex items-center gap-3 text-sm group ${
                    activeConversationId === conv.id 
                      ? 'bg-white dark:bg-white/10 text-[#5A664B] dark:text-[#7E8D6B] shadow-sm border border-gray-100 dark:border-white/5' 
                ...
                >
                  <MessageSquare className={`w-4 h-4 transition-colors ${
                    activeConversationId === conv.id ? 'text-[#5A664B]' : 'text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-200'
                  }`} />
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10">
        <header className="flex items-center px-6 py-4 bg-white/50 dark:bg-black/20 backdrop-blur-md border-b border-gray-200/50 dark:border-white/5 z-20">
          <div className="flex items-center gap-2 flex-1">
            <Sparkles className="w-5 h-5 text-[#5A664B]" />
            <h1 className="text-lg font-semibold bg-gradient-to-r from-[#5A664B] to-[#434B38] dark:from-[#7E8D6B] dark:to-[#5A664B] bg-clip-text text-transparent">
              n8n Agent AI
            </h1>
          </div>
          {user && (
        ...
        {activeConversationId ? (
          <>
            <main ref={scrollContainerRef} className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 scroll-smooth relative">
              {isMessagesLoading ? (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-8 h-8 border-4 border-[#5A664B] border-t-transparent rounded-full animate-spin"></div>
                </div>
              ) : (
                <div className="max-w-4xl mx-auto flex flex-col space-y-8 pb-32">
                  {messages.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-full mt-32 space-y-4">
                      <div className="w-16 h-16 rounded-2xl bg-[#FAFAF8] dark:bg-[#434B38]/20 flex items-center justify-center border border-[#E5E7E1] dark:border-[#5A664B]/20 shadow-sm">
                        <Bot className="w-8 h-8 text-[#5A664B]" />
                      </div>
                  ...
                  {messages.map((msg, idx) => (
                    ...
                    >
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${
                        msg.role === "user" 
                          ? "bg-gradient-to-br from-[#7E8D6B] to-[#434B38] text-white" 
                          : "bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-[#5A664B]"
                      }`}>
                      ...
                      <div
                        className={`max-w-[80%] rounded-2xl px-6 py-4 shadow-sm ${
                          msg.role === "user"
                            ? "bg-gradient-to-br from-[#7E8D6B] to-[#434B38] text-white rounded-tr-none"
                          ...
                          <div className="mb-4">
                            {!msg.isComplete ? (
                              <div className="space-y-2 pb-4 border-b border-gray-100/50 dark:border-white/5">
                                {msg.thinking.map((thought, i) => (
                                  <div key={i} className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-2.5">
                                    <div className="w-1.5 h-1.5 rounded-full bg-[#7E8D6B] animate-pulse shrink-0"></div>
                              ...
                              <details className="text-sm text-gray-500 dark:text-gray-400 cursor-pointer pb-3 mb-3 border-b border-gray-100/50 dark:border-white/5 group">
                                <summary className="font-medium hover:text-[#5A664B] dark:hover:text-[#7E8D6B] select-none outline-none flex items-center gap-2 transition-colors">
                                  <span className="flex items-center justify-center w-4 h-4 rounded bg-gray-100 dark:bg-zinc-800 group-open:bg-[#FAFAF8] dark:group-open:bg-[#434B38]/30">
                                    <Sparkles className="w-2.5 h-2.5" />
                                  </span>
                                  View generation thoughts
                                </summary>
                                <div className="mt-3 pl-3 space-y-1.5 border-l-2 border-[#E5E7E1] dark:border-[#434B38]/50">
                                  {msg.thinking.map((thought, i) => (
                        ...
                        <div className="whitespace-pre-wrap leading-relaxed">
                          {msg.content || (!msg.isComplete && msg.thinking && msg.thinking.length > 0 ? (
                            <div className="flex items-center gap-1 h-6">
                              <div className="w-1.5 h-1.5 rounded-full bg-[#5A664B] animate-bounce" style={{ animationDelay: '0ms' }}></div>
                              <div className="w-1.5 h-1.5 rounded-full bg-[#5A664B] animate-bounce" style={{ animationDelay: '150ms' }}></div>
                              <div className="w-1.5 h-1.5 rounded-full bg-[#5A664B] animate-bounce" style={{ animationDelay: '300ms' }}></div>
                            </div>
                          ) : "")}
                        </div>
                      </div>
                  ...
                  {isLoading && !messages.find(m => !m.isComplete) && (
                    <div className="flex gap-4 flex-row">
                      <div className="w-8 h-8 rounded-full bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-[#5A664B] flex items-center justify-center flex-shrink-0 shadow-sm">
                      ...
                      <div className="max-w-[80%] rounded-2xl px-6 py-5 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border border-gray-200/50 dark:border-white/5 shadow-sm rounded-tl-none flex items-center gap-1.5">
                        <div className="w-2 h-2 rounded-full bg-[#7E8D6B] animate-bounce" style={{ animationDelay: '0ms' }}></div>
                        <div className="w-2 h-2 rounded-full bg-[#7E8D6B] animate-bounce" style={{ animationDelay: '150ms' }}></div>
                        <div className="w-2 h-2 rounded-full bg-[#7E8D6B] animate-bounce" style={{ animationDelay: '300ms' }}></div>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* Input Dock */}
            <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 bg-gradient-to-t from-[#F5F5F0] via-[#F5F5F0]/80 to-transparent dark:from-black dark:via-black/80 z-20 pointer-events-none">
              <div className="max-w-4xl mx-auto pointer-events-auto">
                <form
                  onSubmit={handleSubmit}
                  className="flex items-center bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl rounded-2xl p-2 border border-gray-200/50 dark:border-white/10 shadow-lg focus-within:border-[#5A664B]/50 dark:focus-within:border-[#5A664B]/50 focus-within:ring-4 focus-within:ring-[#5A664B]/10 transition-all"
                >
                  <input
                    type="text"
                    value={input}
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-gray-500 dark:text-gray-400 relative z-10">
            <div className="w-20 h-20 rounded-3xl bg-white/50 dark:bg-white/5 backdrop-blur-xl border border-gray-200/50 dark:border-white/5 shadow-sm flex items-center justify-center mb-6">
              <Bot className="w-10 h-10 text-[#5A664B]" />
            </div>
        ...
        <DialogContent className="sm:max-w-[425px] bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl border-gray-200/50 dark:border-white/10 shadow-2xl">
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              className="bg-gradient-to-r from-[#5A664B] to-[#434B38] hover:from-[#434B38] hover:to-[#33392A] text-white border-0"
            >
